  ...
  _createClass(MainRateChart, [{
    ...
    value: function componentDidMount() {
      ...
      $.getJSON(this.props.chartUrl, function (data) {
        ...
        var scaleX = d3.time.scale().domain([processedData.points[0].x, processedData.points[processedData.points.length - 1].x]).range([width, 40]);
        ...
        d3svg.append('g').attr('class', 'axis').attr('transform', 'translate(40, 0)').call(d3.svg.axis().scale(scaleY).orient('left'));
      });
    }
  }, {
